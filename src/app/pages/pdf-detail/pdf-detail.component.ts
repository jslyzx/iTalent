...
export class PdfDetailComponent extends BaseComponent implements OnInit, OnDestroy {
  pdfSrc = null;
  pdfId;
  ...
  ngOnInit(): void {
    ...
    this.route.paramMap.subscribe(params => {
      ...
      this.documentService.getDocumentById(this.pdfId).subscribe(
        (result: any) => {
      ...
      this.documentService.postDocumentRead(this.pdfId).subscribe(
        (result: any) => {
          console.info(result);
        },
        (error: any) => {
      ...
      this.documentService.getDocumentBase64ByHash(this.pdfId).subscribe(
        (result: any) => {
  ...
  handleComment(id, text) {
    ...
    this.documentService.postDocumentComment(this.pdfId, {id, text}).subscribe(
      (result: any) => {
        const {data, code, message} = result;
        if (code === '1') {
          this.tmpCommentId = 0;
  ...
  handleLike() {
    this.documentService.postDocumentLike(this.pdfId).subscribe(
      (result: any) => {
        const {data, code, message} = result;
        if (code === '1') {
          this.likes = data.likes;
  ...
  private loadComments() {
    this.documentService.getDocumentCommentPublished(this.pdfId).subscribe(
      (result: any) => {
        const {data, code, message} = result;
        if (code === '1') {
          this.comments = data;
